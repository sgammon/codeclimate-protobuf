...
ADD dist/protolint-1.1.1.tar.gz /protolint
...
RUN adduser -u 9000 -D -s /bin/false app \
    && cd /protolint/protolint-1.1.1 && python setup.py build install \
    && mkdir /.linter && chmod 777 /.linter

ENV PYTHONPATH "/protolint/protolint-1.1.1:/protolint/site-packages:/usr/lib/python2.7/site-packages"

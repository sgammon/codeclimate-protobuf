...
ADD dist/protolint-1.1.3.tar.gz /protolint
ADD dist/dependencies.tar.gz /protolint/site-packages
COPY protolint.sh /protolint.sh
RUN adduser -u 9000 -D -s /bin/false app \
    && cd /protolint/protolint-1.1.3 && python setup.py build install \
    && mkdir /.linter && chmod 777 /.linter

ENV PYTHONPATH "/protolint/protolint-1.1.3:/protolint/site-packages:/usr/lib/python2.7/site-packages"
